    ...
    <div class="card">
        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.users.update", [$user->id]) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    <label class="" for="grade">{{ trans('cruds.user.fields.grade') }}</label>
                    <input class="form-control {{ $errors->has('grade') ? 'is-invalid' : '' }}" type="text" name="grade"
                           id="grade" value="{{ old('grade', $user->grade) }}">
                    @if($errors->has('grade'))
                        <div class="invalid-feedback">
                            {{ $errors->first('grade') }}
                        </div>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('unite') ? 'is-invalid' : '' }}" type="text" name="unite"
                           id="unite" value="{{ old('unite', $user->unite) }}">
                    @if($errors->has('unite'))
                        <div class="invalid-feedback">
                            {{ $errors->first('unite') }}
                        </div>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('team') ? 'is-invalid' : '' }}" name="team_id"
                        ...
                        @foreach($teams as $id => $team)
                            <option
                                value="{{ $id }}" {{ ($user->team ? $user->team->id : old('team_id')) == $id ? 'selected' : '' }}>{{ $team }}</option>
                        @endforeach
                    </select>
                    @if($errors->has('team'))
                        <div class="invalid-feedback">
